/*
  ...
  - You are about to drop the column `phone_user` on the `users` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[username_user]` on the table `users` will be added. If there are existing duplicate values, this will fail.
  ...
  - Added the required column `username_user` to the `users` table without a default value. This is not possible if the table is not empty.
...
ADD COLUMN     "username_user" TEXT NOT NULL;
